    ...
    <section id="contact" className="section-padding bg-ivory relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-12 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row gap-6 md:gap-8 lg:items-stretch">
           ...
           {/* Studio Node (Info Panel) - High-Visibility Text Fix */}
              ...
              <div className="bg-emerald p-8 md:p-14 rounded-[2.5rem] md:rounded-[3.5rem] text-white shadow-2xl flex-1 relative overflow-hidden">
                 ...
                 <div className="absolute top-6 right-6 md:top-10 md:right-10">
                    <div className="flex items-center gap-2 px-3 py-1 bg-white/10 rounded-full border border-white/20">
                       ...
                       <span className="text-[8px] font-black uppercase tracking-widest opacity-100">Ready</span>
                    </div>
                 </div>

                 <div className="relative z-10">
                    <span className="text-gold text-[10px] font-black uppercase tracking-[0.5em] mb-6 block">{t.directAccess}</span>
                    
                    {/* FIXED: Increased visibility for "Engage the Studio" */}
                    <h2 className="text-4xl md:text-6xl font-display font-black mb-8 leading-[1.1] tracking-tight text-white">
                       {lang === "en" ? "Engage the" : "ቡድናችንን"} <br />
                       <span className="text-gold italic">{lang === "en" ? "Studio." : "አነጋግሩ።"}</span>
                    </h2>

                    <p className="text-white/80 text-base md:text-lg mb-10 max-w-sm font-medium leading-relaxed">
                       {lang === "en" ? "Immediate consultation with our architectural design team in Addis Ababa." : "በአዲስ አበባ ካለው የዲዛይን ቡድናችን ጋር ቀጥታ ግንኙነት ያድርጉ።"}
                    </p>

                    <div className="space-y-6 md:space-y-8">
                       {[
                         { icon: Phone, label: t.studioLine, val: "+251 911 000 000", color: "text-gold" },
                         { icon: MapPin, label: t.hqNode, val: "Bole Road, Addis Ababa", color: "text-white" },
                         { icon: Clock, label: t.availability, val: "Mon - Sat: 9:00 - 19:00", color: "text-white" },
                       ].map((item, i) => (
                 ...
                 <div className="mt-12 pt-8 border-t border-white/10 hidden md:flex items-center justify-between">
                    <span className="text-[9px] font-black uppercase tracking-widest text-white/20">Studio Terminal v4</span>
                    <div className="flex gap-1">
                       {[1,2,3].map(i => <div key={i} className="w-1.5 h-1.5 rounded-full bg-gold/40" />)}
                    </div>
                 </div>
              </div>
           </div>

           {/* Intake Terminal (Form) */}
           <div className="lg:w-3/5 order-1 lg:order-2">
              <div className="bg-white p-8 md:p-14 lg:p-20 rounded-[2.5rem] md:rounded-[4rem] border border-emerald/10 shadow-xl relative h-full">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mb-10 md:mb-12">
                    <div className="space-y-4 md:space-y-6">
                       ...
                       <input 
                         type="text" 
                         placeholder="Name / ስም"
                         className="w-full text-xl md:text-2xl font-display font-black text-emerald bg-transparent border-b-4 border-sand py-2 focus:border-gold outline-none transition-all placeholder:text-emerald/10"
                       />
                    </div>
                    <div className="space-y-4 md:space-y-6">
                       <label className="flex items-center gap-2 text-[10px] font-black text-emerald uppercase tracking-[0.2em]">
                 ...
                 <div className="space-y-4 md:space-y-6 mb-12 md:mb-16">
                    ...
                    <textarea 
                      placeholder="Goal / ዓላማ"
                      rows={2}
                      className="w-full text-xl md:text-2xl font-display font-black text-emerald bg-transparent border-b-4 border-sand py-2 focus:border-gold outline-none transition-all placeholder:text-emerald/10 resize-none"
                    />
                 </div>
                 ...
                 <div className="flex flex-col sm:flex-row items-center justify-between gap-8 md:gap-10">
                    <div className="flex items-start gap-4">
                       ...
                       <p className="text-[10px] font-black uppercase tracking-widest leading-relaxed text-emerald/30 max-w-[160px]">
                    ...
                    <button 
                      ...
                      className="w-full sm:w-auto px-10 py-6 bg-emerald text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-4 transition-all hover:bg-gold shadow-2xl shadow-emerald/20"
                    >
                       {t.connectStudio} <ArrowRight size={18} />
                    </button>
                 </div>
              </div>
           </div>
